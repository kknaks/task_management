...
const config: Config = {
  ...
  theme: {
    ...
    extend: {
      ...
      /**
       * **줄 행의 첫 줄 상자**(회의록 L858~871 · `LineRow`) — 종류 라벨을 **본문 첫 줄에 맞추는** 값이다.
       *
       * 라벨(11px)과 본문(15px)은 글자 크기가 달라 줄 상자 높이도 다르다. 그대로 두면 라벨 글자가
       * 본문보다 **위로 떠 보인다**(11 × 1.4 = 15.4 vs 15 × 1.65 = 24.75 — 중심이 7.7 vs 12.4).
       * 라벨에 **본문과 같은 줄 상자**를 주면 각자 글자가 그 안에서 가운데로 앉아 첫 줄 기준으로 맞는다.
       * 값은 위 `fontSize` 의 `body`(15 × 1.65) · `meta`(13 × 1.5) 에서 온다 — 그 둘을 고치면 여기도 같이 고친다.
       */
      lineHeight: {
        "row-body": "24.75px",
        "row-body-compact": "19.5px",
      },
