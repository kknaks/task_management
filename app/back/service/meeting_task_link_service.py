...
| `pending_change` 의 세 키를 **순서대로** `task_service` 에 넘긴다 | `task.status` 대입 · `TaskCompletionBlockedError` 포착 · 결과자료 참조 |
...
기한 · 메모가 먼저 반영된 채 남는 상태가 없고 `pending_change` 도 그대로다. 그래서 여기 `try/except` 가 없다.
...
from dto.enums import LineKind, PENDING_CHANGE_STATUSES
...
# `pending_change` JSONB 의 키 — **셋뿐**(DEC-003 §4 L102 · M-14-a). 쓰기 스키마가 이미 거르지만 저장값도 같은 규칙으로 읽는다
...
async def create_task_from_line(
    ...
    await meeting_line_repository.update_line(
        ...
        meeting_id=meeting_id,
        line_id=line.id,
        values={"kind": LineKind.TASK.value, "task_id": task_id, "content": title, "pending_change": None},
    )
    return await meeting_service.build_detail(session, account_id=account_id, meeting_id=meeting_id)


# --- 업무 갱신 (`pending_change` 적용 — SPEC-008 §4 ①②③④) ---------------------------------------------------


def _stored_change(line: MeetingLineDTO) -> dict:
    """줄에 저장된 `pending_change` — 없으면 422(반영할 것이 없다). 키 셋 밖 · `cancelled` 는 저장될 수 없지만 읽을 때도 같은 규칙이다."""
    change = line.pending_change
    if line.kind != LineKind.TASK.value or not change:
        raise _invalid_input("pendingChange")
    if not set(change) <= _PENDING_KEYS:
        raise _invalid_input("pendingChange")
    if "status" in change and change["status"] not in PENDING_CHANGE_STATUSES:
        raise _invalid_input("pendingChange.status")
    return change
...
async def apply_pending_change(
    ...
    """`PATCH …/lines/{id}/task`(U-6 「업무 갱신」) — **본문 없음. 줄의 `pending_change` 가 요청이다.**
    ...
    ④ `pending_change = NULL`
    어느 단계든 예외면 전부 되돌아가고 `pending_change` 가 남는다. 연결 업무가 삭제됐으면 404(§4 Case Matrix `not_found`).
    """
    meeting = await meeting_service.require_meeting(session, account_id=account_id, meeting_id=meeting_id)
    meeting_service.assert_allowed(meeting, "line_edit")
    line = await meeting_edit_service.require_editable_line(session, meeting=meeting, line_id=line_id)
    ...
    await meeting_line_repository.update_line(
        session, meeting_id=meeting_id, line_id=line.id, values={"pending_change": None}
    )
    return await meeting_service.build_detail(session, account_id=account_id, meeting_id=meeting_id)
